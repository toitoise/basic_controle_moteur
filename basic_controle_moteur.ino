...
#include <AccelStepper.h>           // Pour le moteur
...
// Variables traitement vitesse pour se déplacer dans le tableau
...
void setup() {
  ...
  digitalWrite(sleep_n, HIGH);  // Not in Sleep mode
  digitalWrite(enable_n,HIGH);  // Disable... Re-enable later
  digitalWrite(M0,    LOW);
  digitalWrite(M1,    LOW);
  digitalWrite(M2,    LOW);
  ...
  // Set acceleration factor,nombre de steps pour atteindre la vitesse demandee
  myStepper.setAcceleration(100);

  // Active l'interruption sur la pin 2 venant du capteur HALL (D0-pin numerique)
...
void loop() {
  ...
  button_millis_cur = millis();      // utile pour eviter les rebond du boutton     
  // Run Stepper to Target
  myStepper.run();             // DO stepper "run" at each loop to make it turning       

  // check state and add debouncer (500ms pour etre sur que c'est un vrai appui)
  // Le bouton permet de se promener dans le tableau "speed_prog"
  if (button_state == LOW && (button_millis_cur - button_millis_save)>500) {
    ...
    myStepper.setMaxSpeed( speed_prog[motor_speed_cur][0] );    // Nouvelle consigne de vitesse
    myStepper.moveTo(MAX_ROTATION);                             // Combien de steps
  ...
  digitalWrite(enable_n,speed_prog[motor_speed_cur][4]);
    ...
    if ((clk_state_last == LOW) && (clk_state == HIGH)) { // Rising CLK
      // Read rotary Data pin to know which direction increase/decrease new speed
      if (digitalRead(encoderData) == LOW) {
        if (motor_speed_new < (ligneTab-1))     // Test pour ne pas deborder du tableau
          motor_speed_new = motor_speed_new + 1; // On se déplacera de +1 ligne dans le tableau speed_prog
      } else {
        if (motor_speed_new > 0)                // Valeur min 0, tant que > 0 on peut decrementer
          motor_speed_new = motor_speed_new - 1;// On se déplacera de -1 ligne dans le tableau speed_prog
      }
  ...
  // If time update then interrupt occurred (debug)
